...
async function submitSightingUpload(event) {
  ...
  if (!trainLabel || !file) {
    if (elements.sightingStatus) elements.sightingStatus.textContent = "Train and media are required.";
    return;
  }
...
function renderRoutes(routes) {
  ...
  routes.forEach((route) => {
    ...
    const color = route.color || getOperatorColor(route.source);
    ...
    features.push({
      ...
      properties: { color, name: route.name || "", source: route.source || "" },
    });
