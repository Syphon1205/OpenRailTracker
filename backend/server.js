...
const sourceConfigs = {
  ...
  bart: {
    ...
    responseType: "transitstat",
    routeGeoJson: ["https://gtfs.piemadd.com/data/bart/shapes/type_1.geojson"],
  },
  marta: {
    label: "MARTA",
  ...
  metrolink: {
    ...
    vehiclePositionsUrl: "https://gtfs.piemadd.com/data/metrolink/vehiclePositions.pb",
    tripUpdatesUrl: "https://gtfs.piemadd.com/data/metrolink/tripUpdates.pb",
    routeGeoJson: ["https://gtfs.piemadd.com/data/metrolink/shapes/type_2.geojson"],
  },
  caltrain: {
    label: "Caltrain",
    responseType: "gtfs-rt",
    vehiclePositionsUrl: "https://gtfs.piemadd.com/data/caltrain/vehiclePositions.pb",
    tripUpdatesUrl: "https://gtfs.piemadd.com/data/caltrain/tripUpdates.pb",
  ...
  ace: {
    label: "ACE",
    responseType: "gtfs-rt",
    vehiclePositionsUrl: "https://gtfs.piemadd.com/data/ace/vehiclePositions.pb",
    tripUpdatesUrl: "https://gtfs.piemadd.com/data/ace/tripUpdates.pb",
    routeGeoJson: ["https://gtfs.piemadd.com/data/ace/shapes/type_2.geojson"],
  },
  frontrunner: {
    label: "FrontRunner",
    responseType: "gtfs-rt",
...
function geoJsonToRouteFeatures(geojson, source, sampleRate = 3) {
  if (!geojson) return [];
  ...
  const sampleCoords = (coords) => {
    ...
    for (let i = 0; i < coords.length; i++) {
      if (i === 0 || i === coords.length - 1 || i % sampleRate === 0) out.push(coords[i]);
    }
...
// Generic GTFS-RT loader for sources with piemadd.com vehicle positions
async function loadGenericGtfsRtSource(sourceKey) {
  const config = sourceConfigs[sourceKey];
  if (!config?.vehiclePositionsUrl) return [];

  const [posResult, tuResult] = await Promise.allSettled([
    fetchGtfsRt(config.vehiclePositionsUrl),
    config.tripUpdatesUrl ? fetchGtfsRt(config.tripUpdatesUrl) : Promise.resolve(null),
  ]);

  const posData = posResult.status === "fulfilled" ? posResult.value : null;
  const tuData  = tuResult.status  === "fulfilled" ? tuResult.value  : null;
...
async function loadTrains() {
  ...
  const results = await Promise.allSettled([
    ...
    ...gtfsRtKeys.map((key) => loadGenericGtfsRtSource(key)),
    loadDartTrains(),
    loadViaTrains(),
  ]);
...
async function loadRoutes() {
  ...
  const geojsonRequests = Object.entries(sourceConfigs)
    .filter(
      ([key, config]) => Array.isArray(config.routeGeoJson) && key !== "mta" && key !== "njt"
    )
    .flatMap(([key, config]) =>
  ...
  const [viaData, dartData] = await Promise.all([viaDataPromise, dartDataPromise]);
  ...
  viaData.tripsById.forEach((trip) => {
    ...
    const route = viaData.routesById.get(trip.route_id);
    const coords = shape.filter((_, index) => index % 3 === 0).map((p) => [p.lon, p.lat]);
